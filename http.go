...
var httpCmd = &cobra.Command{
	Use:   "serve",
	Short: "Run the CorporateLorem website as a HTTP server.",
	Long:  `Run the CorporateLorem website as a HTTP server. The HTTP server also serves the web API.`,
	Run: func(cmd *cobra.Command, args []string) {
		if useEnvPort {
			bindHost = ":" + os.Getenv("PORT")
		}

		cmd.Printf("Starting HTTP server for CorporateLorem on %v...\n", bindHost)
